...
def normalize_bbb(input_file: str, supplier_reference_file: str) -> str:
    """
    Main function to normalize BBB purchase data
    Args:
        input_file: Path to input CSV/Excel file
        supplier_reference_file: Path to supplier reference Excel file
    Returns:
        Path to output Excel file
    """
    try:
        # Initialize normalizer
        normalizer = BBBNormalizer(supplier_reference_file)
        
        # Process input file
        processed_data = normalizer.process_input_file(input_file)
        
        # Create output sheets with exact format
        purchase_log = normalizer.create_purchase_log_sheet(processed_data)
        item_totals = normalizer.create_item_totals_sheet(purchase_log)
        supplier_totals = normalizer.create_supplier_totals_sheet(purchase_log)
        
        # Generate output filename using utility function
        output_filename = generate_output_filename("BBB_Normalized_Output")
        output_path = os.path.join("files", output_filename)
        
        # Save to Excel
        normalizer.save_to_excel(purchase_log, item_totals, supplier_totals, output_path)
        
        logger.info(f"BBB normalization complete. Output saved to: {output_filename}")
        return output_filename
        
    except Exception as e:
        logger.error(f"Error in BBB normalization: {str(e)}")
        raise 